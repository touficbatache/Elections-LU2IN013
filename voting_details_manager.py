...
class VotingDetails:
    ...
    def show_remaining_methods_details(self, frame: Toplevel):
        ...
        if self.remaining_methods_details[0] is not None:
            if not self.remaining_methods_details[0][1]:
                tk.Label(top_step, text="Le gagnant est : " + self.remaining_methods_details[0][0], font=("Mistral", "15", "bold")).grid(row=row_upgrade, columnspan=nb_candidates if nb_candidates < 12 else 12, column=0)
            else:
                tk.Label(top_step, text="Le gagnant a été départagé :", font=("Mistral", "15", "bold")).grid(row=row_upgrade, columnspan=nb_candidates if nb_candidates < 12 else 12, column=0)
                tk.Label(top_step, text=self.remaining_methods_details[0][0] + " a gagné parmi " + str(self.remaining_methods_details[0][2])).grid(row=row_upgrade + 1, columnspan=nb_candidates if nb_candidates < 12 else 12, column=0)
        else:
    ...
    def show_condorcet_steps(self, frame: Toplevel):
        ...
        if nb_candidates == 1:
            ...
            ttk.Label(scrollable_frame, text="Le gagnant est : " + self.condorcet_details[5][0], font=("Mistral", "15", "bold")).grid(row=1, column=0)
            return
        else:
            ...
            if not self.condorcet_details[0] and not self.condorcet_details[0]:
                ...
                ttk.Label(scrollable_frame, text="On utilise la méthode de " + str.lower(self.condorcet_details[3].name)).grid(row=row_manager + 1, column=0)
                row_manager += 2
                ...
                if self.condorcet_details[0][3] is None:
                    ...
                    ttk.Label(scrollable_frame, text="Plusieurs gagnants : " + str(sorted(self.condorcet_details[6])), font=("Mistral", "15", "bold"), wraplength=300).grid(row=row_manager, column=0)
                    ttk.Label(scrollable_frame, text="On a recours a la méthode de " + str.lower(self.condorcet_details[4].name) + " pour départager entre les différents candidats gagnants", wraplength=300).grid(row=row_manager + 1, column=0)
                    row_manager += 2
                    ttk.Label(scrollable_frame, text=" ").grid(row=row_manager, column=0)
                    row_manager += 1
    ...
    def show_elimination_successive_steps(self, frame: Toplevel):
        ...
        for round_number, scores_letter in self.elimination_successive_details[0].items():
            ...
            if letter != "":
                ...
                ttk.Label(scrollable_frame, text="Candidat éliminé au tour #" + str(round_number) + " : " + letter).grid(row=grid_row_manager, columnspan=nb_candidates if nb_candidates < 12 else 12, column=0)
        ...
        ttk.Label(scrollable_frame, text="Gagnant par majorité (>" + str(self.elimination_successive_details[1]) + ") : " + self.elimination_successive_details[2][0], font=("Mistral", "13", "bold")).grid(row=grid_row_manager, column=0, columnspan=nb_candidates if nb_candidates < 12 else 12)
